...
class Transaksi extends CI_Controller {
    ...
    public function cetak_book_certificate()
    {
        $nim = $this->session->userdata('anggota_id');
        $sql = "SELECT * FROM tbl_login WHERE anggota_id='$nim'";
        ...
        $pdf->Image('https://www.untan.ac.id/wp-content/uploads/2019/04/untan-vektor-large-300x300-300x300.png',28,20,25,0,'PNG');
        $pdf->Cell(25);
        ...
        $pdf->Cell(25);
        $pdf->Cell(0,5,'UNIVERSITAS TANJUNGPURA',0,1,'C');
        $pdf->Cell(25);
        $pdf->Cell(0,5,'FAKULTAS TEKNIK',0,1,'C');
        $pdf->Cell(25);
        $pdf->SetFont('Times','','9');
        $pdf->Cell(0,5,'Jl. Prof.Dr.H.Hadari Nawawi Pontianak 78124 Telepon(0561740186, Kontak Pos 1049',0,1,'C');
        $pdf->Cell(25);
        $pdf->Cell(0,5,'E-mail : ft@untan.ac.id   Homepage http://www.untan.ac.id',0,1,'C');
        $pdf->SetLineWidth(1);
        $pdf->Line(30,52,180,52);
        $pdf->SetLineWidth(0);
        $pdf->Line(30,51,180,51);
        
        $pdf->SetFont('Times','B','12');
        $pdf->Cell(0,20,'SURAT KETERANGAN BEBAS PINJAM BUKU',2,1,'C');
        $pdf->Cell(0,-10,'REFERENSI/PERPUSTAKAAN',0,1,'C');
        $pdf->Ln(20);
        $pdf->SetFont('Times','B','12');
        $pdf->Cell(0,1,'Yang bertanda tangan di bawah ini, bagian Referensi Fakultas Teknik Universitas',0,1,'L');
        $pdf->Ln();
        $pdf->Cell(0,10,'Tanjungpura Pontianak, dengan ini menerangkan bahwa Mahasiswa sbb :',0,1,'L');
        $pdf->Ln();
        $pdf->Cell(40,10,'Nama',0,'L');
        $pdf->Cell(0,10,': '. $member['nama'],0,'L');
        $pdf->Ln();
        $pdf->Cell(40,10,'NIM',0,'L');
        $pdf->Cell(0,10,': '. $member['user'],0,'L');
        $pdf->Ln();
        $pdf->Cell(40,10,'Tempat/Tgl Lahir',0,'L');
        $pdf->Cell(0,10,': '. $member['tempat_lahir'].', '.$member['tgl_lahir'],0,'L');
        $pdf->Ln();
        $pdf->Cell(40,10,'Jurusan/Prog.Studi',0,'L');
        $pdf->Cell(0,10,': '. $member['tempat_lahir'],0,'L');
        $pdf->Ln();
        $pdf->Cell(40,10,'Tahun Akademik',0,'L');
        $pdf->Cell(0,10,': '. $member['tempat_lahir'],0,'L');
        $pdf->Ln();
        $pdf->Cell(40,10,'Alamat',0,'L');
        $pdf->Cell(0,10,': '. $member['alamat'],0,'L');
        $pdf->Ln();
        $pdf->Cell(40,10,'Keterangan',0,'L');
        $pdf->Cell(0,10,': Bahwa yang bersangkutan telah bebas dari peminjaman',0,'L');
        $pdf->ln();
        $pdf->Cell(43,10,'',0,'L');
        $pdf->Cell(0,1,'Buku Referensi/Perpustakaan Fakultas Teknik UNTAN',0,'L');
        $pdf->ln(20);
        $pdf->Cell(90,10,'',0,'L');$pdf->Cell(0,10,'Pontianak, 25 Januari 2022',0,1,'L');
        $pdf->Cell(90,10,'',0,'L');$pdf->Cell(0,10,'Pengelola Referensi',0,1,'L');
        $pdf->Cell(90,10,'',0,'L');$pdf->Cell(0,50,'H. M. Amin',0,1,'L');
        $pdf->Cell(90,10,'',0,'L');$pdf->Cell(0,-40,'NIP. 196602051987011001',0,1,'L');
