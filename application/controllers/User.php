...
class User extends CI_Controller {
    ...
    public function ubah_status($nim, $status)
    {
    	if ($status == 0) {
    		$stat = 1;
    		$this->M_Admin->simpan_qr($nim);
    	}else{
    		$stat = 0;
    	}
        

    	$data = array('qr_code' => $nim.'.png', 'status' => $stat);
    	$this->M_Admin->update_table('tbl_login','user',$nim,$data);
